...
/**
 * Integration test suite for overlay protocol messages.
 * Tests wire format protocol messages for sections 2.3 (MESSAGING_NODES_LIST)
 * and 2.4 (LINK_WEIGHTS).
 * Validates overlay setup, peer list distribution, connection establishment,
 * and link weight assignment as specified in the PDF.
 * 
 * Section 2.3: Tests MESSAGING_NODES_LIST distribution and peer connection
 * establishment.
...
public class OverlayProtocolTest {
     ...
     * Starts a registry and multiple messaging nodes on a random port.
     * 
    ...
    /**
     * Tests MESSAGING_NODES_LIST distribution during overlay setup (Section 2.3).
     * Verifies that nodes receive peer lists and establish connections correctly.
     * 
    ...
    void testMessagingNodesListDistribution() throws Exception {
        ...
        assertThat(orchestrator.waitForRegistryOutput("setup completed with " + CONNECTION_REQUIREMENT + " connections",
                5))
        ...
        assertThat(totalConnectionCount)
                .as("Total connection count across all nodes should be reasonable for CR=" + CONNECTION_REQUIREMENT)
                .isGreaterThan(0);
    }

    /**
     * Tests peer list format in MESSAGING_NODES_LIST (Section 2.3).
     * Validates that peer addresses follow the correct IP:port format.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    /**
     * Tests connection establishment after MESSAGING_NODES_LIST (Section 2.3).
     * Verifies that all nodes establish connections with varying CR values.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    /**
     * Tests bidirectional connection verification (Section 2.3).
     * Validates that connections between peers are properly established
     * and tracked by both endpoints.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    void testBidirectionalConnections() throws Exception {
        ...
        assertThat(nodeConnections)
                .as("Should have connection information for nodes")
                .isNotEmpty();
    }

    /**
     * Tests handling of insufficient nodes for CR (Section 2.3).
     * Verifies that overlay setup fails gracefully when CR >= N.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    void testInsufficientNodesForCR() throws Exception {
        ...
        boolean setupCompleted = orchestrator
                .waitForRegistryOutput("setup completed with " + NODE_COUNT + " connections", 3);
        assertThat(setupCompleted)
                .as("Setup should not complete with CR >= number of nodes")
                .isFalse();

        // Nodes should not establish connections
        for (int nodeId = 0; nodeId < NODE_COUNT; nodeId++) {
            boolean established = orchestrator.waitForNodeOutput(nodeId, "All connections are established", 2);
    ...
    /**
     * Tests LINK_WEIGHTS message distribution (Section 2.4).
     * Verifies that link weights are properly distributed to all nodes
     * after overlay establishment.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    /**
     * Tests link weight format and range validation (Section 2.4).
     * Validates that weights are in the correct format and within
     * the specified range (1-10).
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    void testLinkWeightFormatAndRange() throws Exception {
        ...
        for (String line : output) {
            ...
            if (matcher.find()) {
                ...
                assertThat(weight)
                        .as("Link weight should be between 1 and 10")
                        .isBetween(1, 10);
            }
        }
        ...
        assertThat(weights)
                .as("Should have link weights in output")
                .isNotEmpty();
    ...
    /**
     * Tests link weight uniqueness requirement (Section 2.4).
     * Verifies that each link has a unique weight value as specified in the PDF.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    /**
     * Tests nodes ready for MST computation after weights (Section 2.4).
     * Verifies that all nodes are ready for messaging after receiving link weights.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    /**
     * Tests sending weights before overlay setup (Section 2.4).
     * Verifies that weight assignment fails when attempted before
     * overlay establishment.
     * 
     * @throws Exception if test execution fails
     */
    @Test
    ...
    void testWeightsBeforeOverlay() throws Exception {
        ...
        assertThat(weightsAssigned)
                .as("Weights should not be assigned before overlay setup")
                .isFalse();
    ...
    /**
     * Tests complete overlay protocol sequence (Sections 2.3 & 2.4).
     * Validates the entire workflow of overlay setup followed by
    ...
    void testCompleteOverlayProtocol() throws Exception {
        ...
        for (int nodeId = 0; nodeId < NODE_COUNT; nodeId++) {
            ...
            assertThat(ready)
                    .as("Node " + nodeId + " should be ready after complete protocol")
                    .isTrue();
        }
