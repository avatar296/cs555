...
/**
 * Integration test suite for message integrity and traffic validation. Tests message integrity,
 * correct counts, and summations as specified in PDF Section 4, ensuring no message loss,
 * duplication, or corruption during routing.
 *
 * <p>Validates that message counts match expected values (rounds * 5 per node), summations balance
 * correctly, and relay tracking functions properly.
 */
...
public class MessageIntegrityTest {
  ...
  /**
   * Sets up the test environment with a complete overlay network. Starts a registry and multiple
   * messaging nodes, establishes overlay connections, and distributes link weights for routing.
   *
  ...
  /**
   * Tests message count accuracy (PDF Section 4). Verifies that each node sends exactly rounds * 5
   * messages and that the total sent equals total received across the overlay.
   *
  ...
  void testMessageCount() throws Exception {
    ...
    assertThat(validation.isValid()).as("Traffic summary should be valid").isTrue();
  ...
  /**
   * Tests payload summation integrity (PDF Section 4.2). Verifies that the cumulative sum of all
   * sent payloads exactly matches the cumulative sum of all received payloads, proving no
   * corruption.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests payload value range (PDF Section 4). Verifies that message payloads use the full integer
   * range (-2^31 to 2^31-1) by checking for both positive and negative summation values.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testPayloadRange() throws Exception {
    ...
    orchestrator.sendRegistryCommand("start " + rounds);
    assertThat(orchestrator.waitForRegistryOutput(rounds + " rounds completed", 120)).isTrue();
  ...
  /**
   * Tests message distribution across nodes (PDF Section 4.1). Verifies that messages are
   * distributed across all nodes in the overlay with reasonable variance based on the routing
   * topology.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests relay counter tracking functionality (PDF Section 4.1). Verifies that nodes correctly
   * track and report the number of messages they relay for other nodes in the overlay.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testRelayTracking() throws Exception {
    ...
    assertThat(totalRelayed).as("Should have relay traffic in overlay").isGreaterThan(0);

    // Not all nodes may relay (depending on topology)
    // But at least some should in a typical overlay
    assertThat(nodesWithRelays).as("At least some nodes should relay messages").isGreaterThan(0);
  }

  /**
   * Tests counter reset functionality (PDF Section 2.9). Verifies that all traffic counters are
   * properly reset after sending traffic summaries, ensuring clean state for subsequent messaging
   * rounds.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests for duplicate message prevention (PDF Section 2.6). Verifies that no messages are
   * duplicated during routing by checking that total received count exactly equals total sent
   * count.
   *
   * @throws Exception if test execution fails
   */
