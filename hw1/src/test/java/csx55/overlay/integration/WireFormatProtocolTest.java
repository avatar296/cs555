...
/**
 * Integration test suite for wire format protocol validation. Tests sections 2.1 and 2.2 of the
 * protocol specification covering REGISTER_REQUEST/RESPONSE and DEREGISTER_REQUEST/RESPONSE message
 * formats.
 *
 * <p>Validates exact message formats, protocol compliance, error handling, and state management for
 * node registration and deregistration operations as specified in the assignment PDF.
 */
...
public class WireFormatProtocolTest {
  ...
  /**
   * Sets up the test environment before all tests. Initializes the test orchestrator and starts the
   * registry.
   *
  ...
  /** Cleans up test resources after all tests. Shuts down all nodes and the test orchestrator. */
  @AfterAll
  ...
  /**
   * Tests REGISTER_REQUEST and REGISTER_RESPONSE format (Section 2.1). Verifies that registration
   * messages follow the correct wire format and that nodes receive appropriate success responses.
   *
  ...
  void testRegistrationMessageFormat() throws Exception {
    ...
    assertThat(nodes).as("Registry should have the node registered").hasSize(1);
  }

  /**
   * Tests registration success information string format (Section 2.1). Verifies that the
   * REGISTER_RESPONSE success message includes the correct node count information in the expected
   * format.
   *
   * @throws Exception if test execution fails
   */
  ...
  void testRegistrationSuccessInfoString() throws Exception {
    ...
    for (int e = 0; e < 3; e++) {
      ...
      // Format: "Registration request successful. The number of messaging nodes currently
      // constituting the overlay is (N)"
      Pattern pattern =
          Pattern.compile(
              ".*[Rr]egistration.*successful.*(?:number|count).*nodes?.*(?:is|are)?.*\\(?(\\d+)\\)?.*");
  ...
  /**
   * Tests registration on same host with different ports (Section 2.1). Verifies that multiple
   * nodes on the same host can register successfully with unique port assignments.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testSameHostDifferentPorts() throws Exception {
    ...
    assertThat(uniquePorts).as("Each node should have a unique port").isEqualTo(ports.size());
  }

  /**
   * Tests duplicate registration error response (Section 2.1). Verifies that the registry properly
   * handles duplicate registration attempts and maintains correct node count.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  /**
   * Tests DEREGISTER_REQUEST and DEREGISTER_RESPONSE format (Section 2.2). Verifies that
   * deregistration messages follow the correct wire format and that nodes are properly removed from
   * the registry.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests deregistration of non-registered node error (Section 2.2). Verifies that attempting to
   * deregister a non-existent or already deregistered node is handled gracefully.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testNonRegisteredNodeDeregistration() throws Exception {
    ...
    assertThat(nodes.size()).as("Node should be removed after deregistration").isEqualTo(0);
  }

  /**
   * Tests registry state after multiple registrations and deregistrations. Verifies that the
   * registry correctly maintains state through multiple registration and deregistration operations
   * (Sections 2.1 & 2.2).
   *
   * @throws Exception if test execution fails
   */
  ...
  @Order(7)
  @DisplayName(
      "Test registry state after multiple registrations and deregistrations (Section 2.1 & 2.2)")
  void testRegistryStateManagement() throws Exception {
    ...
    orchestrator.sendRegistryCommand("list-messaging-nodes");
    Thread.sleep(1000);
    assertThat(TestValidator.parseNodeList(orchestrator.getRegistryOutput())).hasSize(5);
    ...
    orchestrator.sendRegistryCommand("list-messaging-nodes");
    Thread.sleep(1000);
    assertThat(TestValidator.parseNodeList(orchestrator.getRegistryOutput())).hasSize(3);
  ...
  /**
   * Tests registration with port number validation (Section 2.1). Verifies that port numbers in
   * registration responses are valid and follow the correct format.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testPortNumberValidation() throws Exception {
    ...
    List<String> nodes = TestValidator.parseNodeList(orchestrator.getRegistryOutput());
    assertThat(nodes).as("Node should be registered").isNotEmpty();
    ...
    for (String node : nodes) {
      ...
      if (parts.length == 2) {
        ...
        assertThat(port).as("Port should be in valid range").isBetween(1, 65535);
      }
    }
  }

  /**
   * Tests rapid registration and deregistration sequence. Verifies that the registry correctly
   * handles rapid cycles of registration and deregistration operations (Sections 2.1 & 2.2).
   *
   * @throws Exception if test execution fails
   */
  @Test
