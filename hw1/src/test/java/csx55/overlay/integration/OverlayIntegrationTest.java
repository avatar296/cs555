...
/**
 * End-to-end integration test suite for the overlay network. Validates complete overlay
 * functionality including node registration, overlay setup, link weight distribution, message
 * routing, and node deregistration.
 *
 * <p>Tests the entire workflow from startup to shutdown, ensuring all components work together
 * correctly as specified in the assignment PDF.
 */
...
public class OverlayIntegrationTest {
  ...
  /**
   * Sets up the complete test system before all tests. Starts the registry and multiple messaging
   * nodes for integration testing.
   *
  ...
  /** Tears down the test system after all tests complete. Shuts down all nodes and the registry. */
  @AfterAll
  ...
  /**
   * Tests node registration functionality. Verifies that all started nodes successfully register
   * with the registry.
   *
  ...
  void testNodeRegistration() throws Exception {
    ...
    assertThat(nodes).as("Should have all nodes registered").hasSize(NODE_COUNT);
  }

  /**
   * Tests overlay setup with specified connection requirement. Verifies that the overlay is
   * established correctly and all nodes successfully establish their required connections.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testOverlaySetup() throws Exception {
    ...
    assertThat(foundSetupMessage).as("Should output correct setup completion message").isTrue();
  }

  /**
   * Tests link weight distribution and verification. Validates that link weights are correctly
   * assigned, distributed to all nodes, and follow the expected format and count.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testLinkWeights() throws Exception {
    ...
    for (int e = 0; e < NODE_COUNT; e++) {
      assertThat(orchestrator.waitForNodeOutput(e, "Link weights received and processed", 10))
  ...
  /**
   * Tests messaging task execution and traffic verification. Validates that nodes correctly send
   * and receive messages, and that traffic summaries show correct counts and summations.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testMessagingTask() throws Exception {
    ...
    assertThat(validation.isValid()).as("Traffic summary should be valid: " + validation).isTrue();
  ...
  /**
   * Tests MST computation at individual nodes. Verifies that nodes correctly compute the Minimum
   * Spanning Tree with exactly N-1 edges and valid properties.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testMSTComputation() throws Exception {
    ...
    assertThat(mstEdges).as("MST should have correct number of edges").hasSize(NODE_COUNT - 1);
  ...
  /**
   * Tests node deregistration functionality. Verifies that nodes can cleanly exit the overlay and
   * that the registry correctly updates its node list.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testDeregistration() throws Exception {
    ...
    assertThat(nodes).as("Should have one less node after deregistration").hasSize(NODE_COUNT - 1);
  }
}
