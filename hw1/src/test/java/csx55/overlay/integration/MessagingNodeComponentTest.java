...
/**
 * Integration test suite for MessagingNode component functionality. Tests MessagingNode-specific
 * behavior from PDF Section 1.2, focusing on node behavior, connection handling, peer
 * identification, and fault tolerance.
 *
 * <p>Validates automatic port selection, registry disconnection handling, peer connection retry
 * logic, MST computation, and graceful node exit.
 */
...
public class MessagingNodeComponentTest {
  ...
  /**
   * Sets up the test environment before each test. Initializes the test orchestrator for managing
   * test nodes.
   *
  ...
  /**
   * Tests automatic port selection functionality (Section 1.2). Verifies that messaging nodes can
   * automatically select available ports when started with port 0, ensuring each node gets a unique
   * valid port.
   *
  ...
  void testAutomaticPortSelection() throws Exception {
    ...
    for (int port : ports) {
      assertThat(port).as("Automatically assigned port should be valid").isBetween(1024, 65535);
    }
  }

  /**
   * Tests node behavior when registry becomes unavailable (Section 1.2). Verifies that messaging
   * nodes detect registry disconnection and terminate appropriately when the registry process is
   * killed.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests invalid command handling at messaging nodes (Section 1.2). Verifies that nodes properly
   * handle invalid or malformed commands and remain responsive after receiving invalid input.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testInvalidCommandHandling() throws Exception {
    ...
    for (String cmd : invalidCommands) {
      ...
      assertThat(foundResponse).as("Node should handle invalid command: " + cmd).isTrue();
    }
  ...
  /**
   * Tests peer connection retry logic (Section 1.2). Verifies that nodes can handle temporary port
   * unavailability and successfully establish connections when ports become available.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  /**
   * Tests cleanup handling for unexpected node shutdown (Section 1.2). Verifies that the registry
   * properly updates its state when nodes terminate unexpectedly and that remaining nodes detect
   * disconnections.
   *
   * @throws Exception if test execution fails
   */
  ...
  void testUnexpectedShutdownCleanup() throws Exception {
    ...
    assertThat(remainingNodes).as("Registry should track only remaining active nodes").hasSize(2);
  ...
  /**
   * Tests duplicate connection handling between peers (Section 1.2). Verifies that nodes properly
   * manage connections to prevent duplicates and maintain the correct number of connections as
   * specified by CR.
   *
   * @throws Exception if test execution fails
   */
  ...
  void testDuplicateConnectionHandling() throws Exception {
    ...
    assertThat(node1HasOneConnection).as("Node 1 should have exactly 1 connection").isTrue();

    assertThat(node2HasOneConnection).as("Node 2 should have exactly 1 connection").isTrue();
  }

  /**
   * Tests peer identification protocol functionality (Section 1.2). Verifies that nodes correctly
   * identify peers after connection establishment and that message routing works properly with peer
   * identification.
   *
   * @throws Exception if test execution fails
   */
  ...
  void testPeerIdentificationProtocol() throws Exception {
    ...
    TestValidator.TrafficSummaryValidation validation =
        TestValidator.validateTrafficSummary(output);
  ...
  /**
   * Tests MST computation after receiving link weights (Section 1.2). Verifies that each node
   * correctly computes the Minimum Spanning Tree using Prim's algorithm and produces MST with
   * exactly N-1 edges.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests graceful exit-overlay functionality (Section 1.2). Verifies that nodes can cleanly exit
   * the overlay, properly deregister from the registry, and that remaining nodes continue to
   * function.
   *
   * @throws Exception if test execution fails
   */
  ...
  void testGracefulExit() throws Exception {
    ...
    assertThat(afterExit).as("Registry should remove exited node").hasSize(2);
