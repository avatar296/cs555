...
/**
 * Integration test suite for overlay command protocol operations. Comprehensive tests for sections
 * 3.1 and 3.2 of the protocol specification.
 *
 * <p>Section 3.1: Tests Registry commands including list-messaging-nodes, list-weights,
 * setup-overlay, send-overlay-link-weights, and start commands.
 *
 * <p>Section 3.2: Tests Messaging node commands including print-mst and exit-overlay.
 *
 * <p>Validates command syntax, output formats, error handling, and state transitions.
 */
...
public class OverlayCommandProtocolTest {
  ...
  /**
   * Sets up the test environment before each test. Starts a registry on a random port to avoid
   * conflicts.
   *
  ...
  /**
   * Tests list-messaging-nodes command with empty registry (Section 3.1). Verifies that the command
   * returns no nodes when registry is empty.
   *
  ...
  /**
   * Tests list-messaging-nodes command with multiple registered nodes (Section 3.1). Verifies
   * correct IP:port format and that all registered nodes are listed.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testListMessagingNodesMultiple() throws Exception {
    ...
    for (String line : output) {
      if (nodePattern.matcher(line.trim()).matches()) {
        ...
        assertTrue(port >= 1024 && port <= 65535, "Port should be in valid range: " + port);
      }
    }
  ...
  /**
   * Tests setup-overlay command with valid connection requirement (Section 3.1). Verifies overlay
   * creation with specified CR and proper connection establishment.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testSetupOverlayValidCR() throws Exception {
    ...
    assertTrue(foundCompletion, "Should output 'setup completed with " + cr + " connections'");
    ...
    assertEquals(6, nodesWithConnections, "All nodes should establish connections");
  }

  /**
   * Tests setup-overlay command with invalid CR >= N (Section 3.1). Verifies that overlay setup
   * fails when CR is greater than or equal to node count.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testSetupOverlayInvalidCR() throws Exception {
    ...
    for (int e = 0; e < 3; e++) {
      boolean established = orchestrator.waitForNodeOutput(e, "All connections are established", 2);
      assertFalse(established, "Node " + e + " should not establish connections with invalid CR");
    }
  }

  /**
   * Tests list-weights command before overlay setup (Section 3.1). Verifies that no weights are
   * listed when overlay hasn't been established.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testListWeightsBeforeSetup() throws Exception {
    ...
    assertEquals(0, weightCount, "Should have no weights before overlay is setup");
  }

  /**
   * Tests list-weights command after overlay setup (Section 3.1). Verifies correct weight format
   * and that all links have weights in range 1-10.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testListWeightsAfterSetup() throws Exception {
    ...
    for (String line : output) {
      ...
      if (matcher.matches()) {
        ...
        assertTrue(weight >= 1 && weight <= 10, "Weight should be between 1-10: " + weight);
      }
    }

    // Should have (N * CR) / 2 unique links
    int expectedLinks = (4 * 2) / 2; // 4 nodes, CR=2
    assertEquals(expectedLinks, links.size(), "Should have correct number of unique links");

    assertFalse(weights.isEmpty(), "Should have weights listed");
  }

  /**
   * Tests send-overlay-link-weights command before overlay setup (Section 3.1). Verifies that
   * weight assignment fails when overlay isn't established.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testSendWeightsBeforeSetup() throws Exception {
    ...
    assertFalse(weightsAssigned, "Should not assign weights before overlay setup");

    // Nodes should not receive weights
    for (int e = 0; e < 3; e++) {
      boolean received = orchestrator.waitForNodeOutput(e, "Link weights received", 2);
      assertFalse(received, "Node " + e + " should not receive weights before overlay setup");
    }
  }

  /**
   * Tests send-overlay-link-weights command after overlay setup (Section 3.1). Verifies successful
   * weight assignment and distribution to all nodes.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testSendWeightsAfterSetup() throws Exception {
    ...
    orchestrator.sendRegistryCommand("setup-overlay 2");
    assertTrue(
        orchestrator.waitForRegistryOutput("setup completed", 5), "Overlay should be setup first");
    ...
    assertTrue(weightsAssigned, "Should output 'link weights assigned'");
    ...
    assertEquals(5, nodesReceivedWeights, "All nodes should receive and process weights");
  }

  /**
   * Tests start command with valid number of rounds (Section 3.1). Verifies task initiation,
  ...
  void testStartCommandValidRounds() throws Exception {
    ...
    // Wait for completion
    assertTrue(
        orchestrator.waitForRegistryOutput(rounds + " rounds completed", 15),
    ...
    assertTrue(foundExactMessage, "Should output exactly '" + rounds + " rounds completed'");
  }

  /**
   * Tests start command with zero rounds edge case (Section 3.1). Verifies proper handling of zero
   * rounds parameter.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests print-mst command output format (Section 3.2). Verifies MST has N-1 edges and follows
  ...
  void testPrintMSTFormat() throws Exception {
    ...
    for (String line : output) {
      ...
      if (matcher.matches()) {
        ...
        int weight = Integer.parseInt(matcher.group(3));
        assertTrue(
            weight >= 1 && weight <= 10, "MST edge weight should be between 1-10: " + weight);
      }
    }
  ...
  /**
   * Tests print-mst command consistency (Section 3.2). Verifies that MST computation is
  ...
  void testPrintMSTConsistency() throws Exception {
    ...
    assertEquals(firstEdges, secondEdges, "MST should be consistent across multiple calls");
  }

  /**
   * Tests exit-overlay command functionality (Section 3.2). Verifies node deregistration and
  ...
  void testExitOverlayCommand() throws Exception {
    ...
    assertTrue(foundExitMessage, "Should output exactly 'exited overlay'");
    ...
    assertEquals(2, remainingNodes.size(), "Should have 2 nodes after one exits");
  }

  /**
   * Tests exit-overlay followed by re-registration (Section 3.2). Verifies that nodes can exit and
   * new nodes can register successfully.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testExitAndReRegister() throws Exception {
    ...
    assertTrue(orchestrator.waitForNodeOutput(0, "exited overlay", 5), "Node should exit overlay");
    ...
    assertEquals(2, afterReRegister.size(), "Should have 2 nodes after re-registration");
  }

  /**
   * Tests complete command sequence workflow (Sections 3.1 & 3.2). Validates end-to-end
   * functionality including node registration, overlay setup, weight assignment, messaging, MST
   * computation, and node exit.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testCompleteCommandSequence() throws Exception {
    ...
    orchestrator.sendRegistryCommand("setup-overlay 2");
    assertTrue(
        orchestrator.waitForRegistryOutput("setup completed with 2 connections", 5),
        "Should setup overlay with CR=2");

    // 3. Send weights
    orchestrator.sendRegistryCommand("send-overlay-link-weights");
    assertTrue(
        orchestrator.waitForRegistryOutput("link weights assigned", 5),
    ...
    assertTrue(
        orchestrator.waitForRegistryOutput("2 rounds completed", 10), "Should complete 2 rounds");

    // 7. Exit one node
    Thread.sleep(20000); // Wait for traffic summary
    orchestrator.sendNodeCommand(3, "exit-overlay");
    assertTrue(orchestrator.waitForNodeOutput(3, "exited overlay", 5), "Node should exit overlay");
