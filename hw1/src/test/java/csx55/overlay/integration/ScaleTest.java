...
/**
 * Integration test suite for overlay scalability testing. Tests the overlay network with different
 * scales and connection requirements, validating that the system works correctly with varying
 * numbers of nodes and different CR values.
 *
 * <p>Includes tests for minimum overlay configurations, standard configurations, large-scale
 * deployments, and edge cases like fully connected graphs.
 */
...
public class ScaleTest {
  ...
  /**
   * Tests minimum viable overlay configuration. Verifies that a 3-node overlay with CR=2 works
   * correctly, representing the smallest functional overlay.
   *
  ...
  void testMinimumOverlay() throws Exception {
    ...
    assertThat(
            orchestrator.waitForRegistryOutput("setup completed with " + cr + " connections", 10))
    ...
    assertThat(linkCount).as("Should have correct number of edges").isEqualTo(expectedEdges);

    // Run a small messaging task
    orchestrator.sendRegistryCommand("start 5");
    assertThat(orchestrator.waitForRegistryOutput("5 rounds completed", 20)).isTrue();
  }

  /**
   * Tests standard overlay configuration as per PDF default. Validates a 10-node overlay with CR=4,
   * which is the standard configuration used in most examples.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testStandardOverlay() throws Exception {
    ...
    orchestrator.sendRegistryCommand("setup-overlay " + cr);
    assertThat(
            orchestrator.waitForRegistryOutput("setup completed with " + cr + " connections", 15))
        .isTrue();
    ...
    int expectedEdges = (nodeCount * cr) / 2;
    assertThat(linkCount).as("10 nodes with CR=4 should have 20 edges").isEqualTo(expectedEdges);
  }

  /**
   * Tests large-scale overlay deployment. Verifies that the system scales to handle 20 nodes with
   * CR=4, testing performance with increased network size.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testLargeOverlay() throws Exception {
    ...
    orchestrator.sendRegistryCommand("setup-overlay " + cr);
    assertThat(
            orchestrator.waitForRegistryOutput("setup completed with " + cr + " connections", 20))
        .isTrue();

    // Send link weights
    orchestrator.sendRegistryCommand("send-overlay-link-weights");
    assertThat(orchestrator.waitForRegistryOutput("link weights assigned", 10)).isTrue();
  ...
  /**
   * Tests fully connected overlay configuration. Verifies that an overlay with CR = N-1 creates a
   * fully connected graph where every node is connected to every other node.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  /**
   * Tests overlay with varying connection requirements. Validates that the system correctly handles
   * different CR values (2, 3, 5, 6) with the same number of nodes.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  /**
   * Tests overlay with odd number of nodes and odd CR. Verifies that the system correctly handles
   * edge cases where both node count and CR are odd numbers, ensuring proper graph construction.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testOddNodesOddCR() throws Exception {
    ...
    assertThat(linkCount).as("Should handle odd node count with odd CR").isGreaterThan(0);

    // Test messaging works
    orchestrator.sendRegistryCommand("start 5");
    assertThat(orchestrator.waitForRegistryOutput("5 rounds completed", 30)).isTrue();
  }
}
