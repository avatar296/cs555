...
/**
 * Integration test suite for validating command output formats. Tests that all command outputs
 * match the exact format specified in the assignment PDF, ensuring consistency and compliance with
 * project requirements.
 *
 * <p>Tests cover both registry commands (Section 3.1) and messaging node commands (Section 3.2),
 * validating output format for list commands, setup commands, and status messages.
 */
...
public class CommandOutputTest {
  ...
  /**
   * Sets up the test environment by starting a registry and multiple messaging nodes.
   *
  ...
  /**
   * Tests the list-messaging-nodes command output format. Validates that node listing follows the
   * IP:port format specified in PDF Section 3.1.
   *
  ...
  /**
   * Tests the setup-overlay command output format. Validates that the setup completion message
   * matches the format specified in PDF Section 3.1.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  /**
   * Tests the send-overlay-link-weights command output format. Validates that the link weights
   * assignment message matches PDF Section 3.1 specifications.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testSendOverlayLinkWeightsOutput() throws Exception {
    ...
    assertThat(foundExactMessage).as("Should output exactly 'link weights assigned'").isTrue();
  }

  /**
   * Tests the list-weights command output format. Validates that link weight listings follow the
   * "nodeA, nodeB, weight" format and that weights are within the valid range (1-10).
   *
   * @throws Exception if test execution fails
   */
  @Test
    ...
    for (String line : output) {
      if (weightPattern.matcher(line.trim()).matches()) {
        ...
        if (parts.length == 3) {
          ...
          assertThat(weight).as("Link weight should be between 1-10").isBetween(1, 10);
        }
      }
    }
  ...
  /**
   * Tests the start command output format. Validates that task completion message follows the "N
   * rounds completed" format.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testStartCommandOutput() throws Exception {
    ...
    assertThat(foundExactMessage).as("Should output exactly 'N rounds completed'").isTrue();
  }

  /**
   * Tests the traffic summary table output format. Validates that the traffic summary follows the
   * table format specified in PDF Section 4.3, including individual node statistics and the sum
   * line.
   *
   * @throws Exception if test execution fails
   */
  ...
  void testTrafficSummaryFormat() throws Exception {
    ...
    for (String line : output) {
      ...
      if (line.startsWith("sum ")) {
        ...
        assertThat(parts).as("Sum line should have 5 parts").hasSize(5);
    ...
    assertThat(foundSumLine).as("Should have sum line at end of traffic summary").isTrue();
  }

  /**
   * Tests the print-mst command output format. Validates that MST output follows the same format as
   * list-weights and contains exactly N-1 edges for N nodes.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testPrintMSTFormat() throws Exception {
    ...
    assertThat(mstEdges).as("MST should have N-1 edges").isEqualTo(NODE_COUNT - 1);
  }

  /**
   * Tests the exit-overlay command output format. Validates that the exit confirmation message
   * matches PDF Section 3.2 specifications.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testExitOverlayOutput() throws Exception {
    ...
    assertThat(foundExactMessage).as("Should output exactly 'exited overlay'").isTrue();
  }

  /**
   * Tests node console messages during overlay setup. Validates that nodes output correct
   * connection establishment and link weights received messages as specified in the PDF.
   *
   * @throws Exception if test execution fails
   */
  @Test
