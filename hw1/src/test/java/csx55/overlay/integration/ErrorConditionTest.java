...
/**
 * Integration test suite for error conditions and edge cases. Tests various error scenarios and
 * boundary conditions as specified in the assignment PDF, ensuring the system handles failures
 * gracefully and maintains consistency.
 *
 * <p>Covers error cases including duplicate registration (Section 2.1), invalid deregistration
...
public class ErrorConditionTest {
  ...
  /**
   * Sets up a fresh test environment before each test. Starts a new registry on a random port to
   * avoid conflicts.
   *
  ...
  /**
   * Tests duplicate registration handling (PDF Section 2.1). Verifies that the registry properly
   * handles multiple registration attempts from nodes and maintains a consistent node list.
   *
  ...
  /**
   * Tests deregistration of non-registered nodes (PDF Section 2.2). Verifies that attempting to
   * deregister a node that has already been deregistered is handled gracefully without system
   * failure.
   *
   * @throws Exception if test execution fails
   */
   ...
   * Verifies that the system properly rejects overlay setup when there are not enough nodes to
   * satisfy the specified connection requirement (CR).
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testInsufficientNodesForCR() throws Exception {
    ...
    assertThat(setupCompleted).as("Setup should not complete with insufficient nodes").isFalse();
  }

  /**
   * Tests IP address validation during registration (PDF Section 2.1). Verifies that nodes must
   * register with their actual IP address and that the registry validates the source IP matches the
  ...
  /**
   * Tests command execution before overlay setup. Verifies that commands requiring an established
   * overlay (like sending link weights or starting messaging tasks) fail gracefully when executed
  ...
  /**
   * Tests overlay setup with zero connection requirement edge case. Verifies that the system
   * handles CR=0 appropriately, either rejecting it or creating an overlay with no connections
   * between nodes.
   *
   * @throws Exception if test execution fails
   */
  ...
  /**
   * Tests node failure during registration process (PDF Section 2.1 NOTE). Simulates the rare case
   * where a node fails immediately after sending a registration request, verifying that the
   * registry handles this gracefully and maintains system stability.
   *
   * @throws Exception if test execution fails
   */
  @Test
  ...
  void testNodeFailureDuringRegistration() throws Exception {
    ...
    List<String> output = orchestrator.getRegistryOutput();
    assertThat(output).as("Registry should still be responsive after node failure").isNotNull();

    // Registry should have cleaned up the failed node
    List<String> nodes = TestValidator.parseNodeList(output);
    assertThat(nodes).as("Registry should maintain valid node list after failure").isNotNull();
  }
}
