...
/** Orchestrates the lifecycle of Registry and MessagingNode processes for testing */
public class TestOrchestrator {
  ...
  public void startRegistry(int port) throws IOException, InterruptedException {
    ...
    ProcessBuilder pb =
        new ProcessBuilder(
            "java", "-cp", classpath, "csx55.overlay.node.Registry", String.valueOf(port));
    ...
    if (!started) {
      ...
      throw new IOException(
          "Registry failed to start on port " + port + " - no confirmation message received");
    }
  ...
  public int startMessagingNode(String registryHost, int registryPort)
    ...
    if (!registered) {
      ...
      System.out.println(
          "[TestOrchestrator] Warning: MessagingNode " + nodeId + " registration not confirmed");
    }
  ...
  private void killProcessOnPort(int port) {
    try {
      ...
      System.out.println(
          "No process found using port " + port + " or unable to kill: " + e.getMessage());
    }
  }
  ...
  private boolean waitForOutput(
      List<String> outputList, String expectedOutput, int timeoutSeconds) {
    long startTime = System.currentTimeMillis();
