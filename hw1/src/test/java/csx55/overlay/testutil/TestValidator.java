...
public class TestValidator {
    ...
    public static boolean validateRegistrationResponse(String output) {
        String pattern = "Registration request successful\\. The number of messaging nodes currently constituting the overlay is \\(\\d+\\)";
    ...
    public static TrafficSummaryValidation validateTrafficSummary(List<String> outputs) {
        ...
        Pattern sumPattern = Pattern.compile(
            "sum\\s+(\\d+)\\s+(\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)"
        );
    ...
    public static class TrafficSummaryValidation {
        ...
        public void addNodeSummary(String nodeId, long sent, long received, 
                                  double sumSent, double sumReceived, int relayed) {
            nodeSummaries.add(new NodeSummary(nodeId, sent, received, sumSent, sumReceived, relayed));
        }
        
        public boolean isValid() {
            // Check if totals match
            boolean totalsMatch = actualTotalSent == expectedTotalSent &&
                                actualTotalReceived == expectedTotalReceived;
            
            // Check if sums match (with small epsilon for floating point)
            double epsilon = 0.01;
            boolean sumsMatch = Math.abs(actualSumSent - expectedSumSent) < epsilon &&
        ...
        public String toString() {
            return String.format(
                ...
                actualTotalSent, expectedTotalSent,
                actualTotalReceived, expectedTotalReceived,
                actualSumSent, expectedSumSent,
                actualSumReceived, expectedSumReceived,
                isValid()
            );
        }
    }
    ...
    public static class NodeSummary {
        ...
        public NodeSummary(String nodeId, long sent, long received, 
                          double sumSent, double sumReceived, int relayed) {
            this.nodeId = nodeId;
            this.sent = sent;
