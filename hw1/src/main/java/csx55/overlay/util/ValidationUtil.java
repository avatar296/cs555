...
/**
 * Utility class for comprehensive input validation and parameter checking. Provides validation
 * methods for network parameters, data integrity, and security-related input sanitization.
 */
public class ValidationUtil {
  ...
  /**
   * Validates IP address format. Checks for proper IPv4 format with valid octets.
   *
  ...
  /**
   * Validates node ID format. Expected format: IP:port
   *
  ...
  /**
   * Validates connection requirement for overlay creation. Checks mathematical feasibility and
   * balance constraints.
   *
  ...
  public static String validateConnectionRequirement(int cr, int nodeCount) {
    ...
    if (nodeCount <= cr) {
      return "Connection requirement ("
          + cr
          + ") must be less than number of nodes ("
          + nodeCount
          + ")";
    }
    ...
    if (totalEdgesNeeded > maxPossibleEdges) {
      return "Impossible to create overlay: requires "
          + totalEdgesNeeded
          + " edges but maximum possible is "
          + maxPossibleEdges;
    }

    if ((nodeCount % 2 == 1) && (cr % 2 == 1) && cr > 1) {
      return "Warning: With odd number of nodes ("
          + nodeCount
          + ") and odd CR ("
          + cr
          + "), overlay may not be perfectly balanced";
    }
  ...
  /**
   * Performs safe addition with overflow protection. Caps result at Long.MAX_VALUE if overflow is
   * detected.
   *
   * @param a first value
  ...
  public static long safeAdd(long a, long b) {
    if (willOverflow(a, b)) {
      LoggerUtil.warn(
          "ValidationUtil", "Integer overflow detected in statistics: " + a + " + " + b);
  ...
  /**
   * Validates hostname format. Accepts both DNS hostnames and IP addresses.
   *
  ...
  /**
   * Sanitizes string input to prevent injection attacks. Removes control characters and limits
   * length.
   *
