...
/**
 * Implements retry logic for network connections with exponential backoff. Provides configurable
 * retry attempts and delays to handle transient network failures. Supports both direct socket
 * connections and generic retryable operations.
 */
public class ConnectionRetryPolicy {
   ...
   */
  private <T> T retryWithTemplate(
      String operationName, RetryableOperation<T> operation, boolean checkRetryable)
      throws Exception {
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        boolean shouldRetry = attempt < maxAttempts && (!checkRetryable || isRetryable(ex));

        if (shouldRetry) {
          long delay = calculateBackoffDelay(attempt);
          LoggerUtil.debug("ConnectionRetry", String.format("Waiting %dms before retry", delay));
    ...
    throw new Exception(
        String.format("%s failed after %d attempts", operationName, lastAttempt), lastException);
  }

  /**
   * Attempts to connect to the specified host and port with retry logic. Uses exponential backoff
   * between attempts.
   *
  ...
  public Socket connectWithRetry(String host, int port) throws IOException {
    ...
    try {
      return retryWithTemplate(operationName, () -> new Socket(host, port), false);
    } catch (Exception e) {
      ...
      LoggerUtil.error(
          "ConnectionRetry",
          String.format("Non-IOException caught: %s - %s", e.getClass().getName(), e.getMessage()));
      ...
      throw new IOException(
          String.format(
              "Connection failed due to %s: %s", e.getClass().getSimpleName(), e.getMessage()),
          e);
    }
  }

  /**
   * Executes an operation with retry logic. Only retries if the exception is determined to be
   * retryable.
   *
   * @param <T> the return type of the operation
   * @param operation the operation to execute
  ...
  /**
   * Calculates the exponential backoff delay for the given attempt. Delay doubles with each
  ...
  /**
   * Determines if an exception is retryable. Network timeouts, connection refused, and connection
   * resets are considered retryable.
   *
