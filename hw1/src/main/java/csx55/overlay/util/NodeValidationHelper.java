...
/**
 * Helper class for common node validation operations.
...
public final class NodeValidationHelper {
    ...
    public static ValidationResult validateIpMatch(String claimedIp, Socket socket) {
        ...
        if (!claimedIp.equals(actualAddress)) {
            String errorMsg = String.format("IP mismatch (claimed: %s, actual: %s)", 
                claimedIp, actualAddress);
    ...
    public static ValidationResult validatePort(int port) {
        if (!ValidationUtil.isValidPort(port)) {
            String errorMsg = String.format("Invalid port number (must be %d-%d)", 
     ...
     */
    public static ValidationResult validateNodeExists(String nodeId, Map<String, TCPConnection> registeredNodes) {
     ...
     */
    public static ValidationResult validateNodeDoesNotExist(String nodeId, Map<String, TCPConnection> registeredNodes) {
        if (registeredNodes.containsKey(nodeId)) {
