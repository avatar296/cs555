...
/**
 * Service responsible for routing messages through the overlay network. Handles both direct message
 * sending and message relay operations while maintaining statistics for all routing activities.
 *
 * <p>This service uses a routing table to determine the next hop for messages and leverages TCP
 * connections for reliable message delivery.
 */
public class MessageRoutingService {
  ...
  public MessageRoutingService(
      TCPConnectionsCache peerConnections, NodeStatisticsService statisticsService) {
  ...
  /**
   * Sends a message to the specified destination node. This method increments send statistics and
   * routes the message through the appropriate next hop.
   *
  ...
  /**
   * Relays a message from a source node to a destination node. This method is used when this node
   * acts as an intermediate hop in the message routing path.
   *
  ...
  /**
   * Internal method to handle common message routing logic. Validates the route, retrieves the
   * connection, and sends the message.
   *
  ...
  private void routeMessageInternal(String sourceId, String sinkId, int payload, String operation) {
    ...
    Optional<TCPConnection> connectionOpt =
        MessageRoutingHelper.getValidatedConnection(
            peerConnections, nextHop, String.format("%s to destination: %s", operation, sinkId));
  ...
  /**
   * Handles incoming data messages. If the message is destined for this node, it updates receive
   * statistics. Otherwise, it relays the message to the next hop.
   *
