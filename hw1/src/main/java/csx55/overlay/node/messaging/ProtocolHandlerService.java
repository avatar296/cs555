...
public class ProtocolHandlerService {
     ...
     */
    public void handlePeerList(MessagingNodesList peerList, TCPConnection.TCPConnectionListener listener)
            throws IOException {
        ...
        if (!connectionStatusPrinted) {
            if (peerList.getPeerNodes().isEmpty()) {
                try {
                    Thread.sleep(1000);
            ...
            LoggerUtil.info("ProtocolHandler",
    ...
    public synchronized void handlePeerIdentification(PeerIdentification identification, TCPConnection connection) {
        ...
        // Check if this connection object is already in the cache (possibly with a
        // different ID)
        boolean found = false;
        ...
        if (!found) {
            peerConnections.addConnection(peerId, connection);
            LoggerUtil.debug("ProtocolHandler",
                    "Peer connection received from " + peerId + ". Total connections: " + peerConnections.size());
        }
    }
