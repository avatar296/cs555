...
/**
 * Service responsible for handling task execution in the MessagingNode. Manages the execution of
 * messaging tasks, including sending messages to random nodes and reporting task completion to the
 * registry.
 */
public class TaskExecutionService {
  ...
  public TaskExecutionService(
      ...
      NodeStatisticsService statisticsService) {
  ...
  public void setNodeInfo(
      String nodeId, String ipAddress, int portNumber, TCPConnection registryConnection) {
    this.nodeId = nodeId;
  ...
  /**
   * Handles a task initiation request from the registry. Resets statistics and starts the messaging
   * task asynchronously.
   *
  ...
  /**
   * Performs the messaging task by sending messages to random nodes. Each round consists of sending
   * 5 messages to randomly selected nodes. Upon completion, sends a task complete message to the
   * registry.
   *
  ...
  private void performMessagingTask(int rounds, List<String> allNodes) {
    LoggerUtil.info(
        ...
        "Starting messaging task for " + rounds + " rounds with " + allNodes.size() + " nodes");

    if (registryConnection == null) {
      LoggerUtil.error(
          "TaskExecutionService", "Registry connection is null, cannot send task complete");
      return;
    }
    ...
    if (allNodes.isEmpty()) {
      LoggerUtil.error(
          "TaskExecutionService",
          "AllNodes list is empty, cannot send messages. Sending TaskComplete anyway.");
    ...
    if (allNodes.size() <= 1) {
      LoggerUtil.warn(
          "TaskExecutionService", "Not enough nodes to send messages to. Skipping task.");
      sendTaskComplete("because no other nodes were available");
      return;
    }
    ...
    LoggerUtil.info(
        "TaskExecutionService", "Completed all rounds, sending TaskComplete to registry");
    sendTaskComplete("after completing all rounds");
  }

  /**
   * Helper method to send a TaskComplete message to the registry. Uses the MessageRoutingHelper for
  ...
  private void sendTaskComplete(String context) {
    ...
    boolean success =
        MessageRoutingHelper.sendEventSafely(
            registryConnection, complete, String.format("sending TaskComplete %s", context));
