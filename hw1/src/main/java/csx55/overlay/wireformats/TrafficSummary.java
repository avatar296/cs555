...
/**
 * Message containing traffic statistics from a messaging node. Sent in response to a
 * PullTrafficSummary request, includes counts and sums for sent, received, and relayed messages.
 *
 * <p>Wire format: - int: message type (TRAFFIC_SUMMARY) - String: node IP address - int: node port
 * number - int: messages sent count - long: sum of sent message payloads - int: messages received
 * count - long: sum of received message payloads - int: messages relayed count
 */
public class TrafficSummary extends AbstractEvent {
  ...
  public TrafficSummary(
      ...
      int nodePortNumber,
      int messagesSent,
      long sumSentMessages,
      int messagesReceived,
      long sumReceivedMessages,
      int messagesRelayed) {
