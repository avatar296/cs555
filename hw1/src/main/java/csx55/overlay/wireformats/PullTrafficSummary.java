...
/**
 * Request message sent by the registry to pull traffic statistics from nodes. This message
 * instructs nodes to report their message traffic summaries including sent/received counts and
 * payload sums.
 *
 * <p>Wire format: - int: message type (PULL_TRAFFIC_SUMMARY)
 */
public class PullTrafficSummary extends AbstractEvent {
  ...
  /**
   * Writes the PullTrafficSummary-specific data to the output stream. This message has no
   * additional data beyond the type.
   *
  ...
  /**
   * Reads the PullTrafficSummary-specific data from the input stream. This message has no
   * additional data beyond the type.
   *
