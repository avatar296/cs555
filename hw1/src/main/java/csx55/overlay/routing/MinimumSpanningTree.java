...
/**
 * Manages Minimum Spanning Tree (MST) calculations and operations for network routing.
 * Uses Prim's algorithm to build an MST from a weighted graph, providing optimal
 * paths from a root node to all other nodes in the network.
 * 
 * This class encapsulates all MST-related logic including tree construction,
 * path finding, and tree traversal operations.
 */
public class MinimumSpanningTree {
    
    /** The identifier of the root node for the MST */
    private final String rootNodeId;
    
    /** Graph representation as adjacency list */
    private final Map<String, List<Edge>> graph;
    
    /** MST mapping each node to its parent edge */
     ...
     * The resulting MST provides optimal paths from the root node to all other nodes.
     * 
     * @return true if MST was calculated successfully, false otherwise
     */
    public boolean calculate() {
        if (!graph.containsKey(rootNodeId)) {
            LoggerUtil.warn("MinimumSpanningTree", 
     ...
     * @return list of nodes in the path from destination to root (exclusive of root),
