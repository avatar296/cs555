...
/**
 * Abstract base class for command-line interface handlers. Provides common functionality for
 * interactive command loops used by both Registry and MessagingNode command handlers.
 *
 * <p>Subclasses must implement the processCommand method to handle specific commands for their node
 * type.
 */
public abstract class AbstractCommandHandler {
  ...
  /**
   * Starts the interactive command loop. Reads user input and delegates command processing to
   * subclasses. Handles resource cleanup when the loop terminates.
   */
  ...
  /**
   * Processes a command input from the user. Subclasses must implement this method to handle their
   * specific commands.
   *
  ...
  /** Stops the command loop. Sets the running flag to false to terminate the loop gracefully. */
  public void stop() {
    running = false;
  }
   ...
   * @return an array where index 0 is the command and remaining indices are arguments
   */
