...
public class BronzeTableSetup {
  ...
  private void createTableForStream(StreamConfig.BronzeStreamConfig streamConfig) {
    ...
    if (sampleBatch.isEmpty()) {
      logger.warn(
          "No data available in topic {}. Table will be created when data becomes available.",
          streamConfig.topic);
      return;
    }

    Dataset<Row> transformedSample =
        AvroUtils.prepareBronzeData(sampleBatch, streamConfig.topic, config.getSchemaRegistryUrl());
    ...
    String createTableDDL =
        generateCreateTableSQL(streamConfig.table, transformedSample, partitionSpec);
    logger.debug("Executing DDL: {}", createTableDDL);
    spark.sql(createTableDDL);

    transformedSample.write().format("iceberg").mode("append").saveAsTable(streamConfig.table);
  ...
  private String generateCreateTableSQL(
      String tableName, Dataset<Row> sample, String partitionSpec) {
