 ...
 */
public class SyntheticSpecialEventGenerator {
    ...
    private SpecialEvent generateRandomEvent(long timestamp) {
        int locationId = 1 + random.nextInt(263);  // Random NYC zone
    ...
    private EventType generateEventType() {
        ...
        for (int i = 0; i < EVENT_TYPE_WEIGHTS.length && i < types.length; i++) {
            cumulative += EVENT_TYPE_WEIGHTS[i];
            if (rand < cumulative) {
                return types[i];
            }
        }
