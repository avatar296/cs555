  ...
  public static class StreamConfig {
    ...
    public String toString() {
      ...
      return String.format(
          "topic=%s, rate=%.1f/s, errors=%.1f%%, total=%s",
          topicName, arrivalRate, errorRate * 100, totalEvents == -1 ? "infinite" : totalEvents);
    }
  }
  ...
  private void validateStream(String name, StreamConfig config) {
    if (config.arrivalRate < 0) {
      throw new IllegalArgumentException(
          name + "_RATE must be non-negative, got: " + config.arrivalRate);
    }
