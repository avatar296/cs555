#!/usr/bin/env python3
"""
Bronze Layer Events Consumer

Consumes NYC event data from Kafka and writes to bronze layer in Iceberg.
Preserves raw event data exactly as received with ingestion timestamp-based partitioning.
...
from ..config.events import EventsConsumerConfig


class BronzeEventsConsumer(BaseBronzeConsumer):
    """Bronze layer consumer for NYC events data."""

    def __init__(self, config: EventsConsumerConfig = None):
        """
        Initialize bronze events consumer.

        Args:
            config: Optional EventsConsumerConfig instance
        """
        # Use provided config or create default
        self.config = config or EventsConsumerConfig()
    ...
    def get_consumer_config(self) -> Dict[str, Any]:
        """
        Get events consumer configuration.
    ...
    def process_batch(self, df: DataFrame) -> DataFrame:
        """
        Process a batch of event records.
...
def main():
    """Main entry point for bronze events consumer."""
    consumer = BronzeEventsConsumer()
    consumer.run()
