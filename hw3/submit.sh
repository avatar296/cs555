...
# HW3 Submission Script
# Creates a clean tar file for CS555 submission

echo "Creating CS555 HW3 submission tar file..."
...
TAR_FILE="build/distributions/Christopher_Cowart_HW3.tar"
...
if [ -f "$TAR_FILE" ]; then
    ...
    FORBIDDEN=$(tar -tf "$TAR_FILE" | grep -E '(\.jar$|bin/hw3$|bin/hw3\.bat$|lib/)' || true)
    if [ -n "$FORBIDDEN" ]; then
